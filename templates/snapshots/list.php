...
(function () {
    ...
    Alpine.data('htwSnapshots', function () {
        return {
            snapshots:    window._htwSnapshots || [],
            scheduled:    false,
            nextRunStr:   window._htwNextRun || null,
            lastRun:      null,
            lastStatus:   window._htwSnapshotStatus || null,
            creating:     false,
            restoring:    false,
            restoreModal: false,
            restoreTarget: null,
            ...
            init: function () {
                this.lastRun = (this.lastStatus && this.lastStatus.run_at) ? this.fmtDate(this.lastStatus.run_at) : null;
                this.fetchStatus();
            },
            ...
            createSnapshot: function () {
                var self = this;
                this.creating = true;
                jQuery.post(HTW.ajaxUrl, { action: 'htw_snapshot_create', nonce: HTW.nonce }, function (res) {
                    self.creating = false;
                    if (res.success) {
                    ...
                    self.creating = false;
                    self.showToast('Tạo snapshot thất bại.', 'error');
                });
            },
            ...
            doRestore: function () {
                ...
                var self = this;
                this.restoring = true;
                jQuery.post(HTW.ajaxUrl, {
                    action: 'htw_snapshot_restore',
                    nonce: HTW.nonce,
                    ...
                    if (res.success) {
                        self.showToast('Khôi phục thành công! ' + res.data.message);
                    } else {
                        self.showToast(res.data || 'Khôi phục thất bại.', 'error');
                    }
                }).fail(function () {
                    self.restoring = false;
                    self.showToast('Khôi phục thất bại.', 'error');
                });
            },
            ...
            deleteSnapshot: function (s) {
                ...
                jQuery.post(HTW.ajaxUrl, {
                    ...
                    self.showToast('Xoá thất bại.', 'error');
                });
            },
